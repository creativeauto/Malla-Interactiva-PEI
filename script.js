...
const puede = n => (requisitos[n]?.requiere || []).every(r => aprobado(r));

function toggleInfo(cont, nombre){
  const box = cont.querySelector(".info-box");
  const d = requisitos[nombre] || { requiere:[], esRequisitoDe:[] };
  box.innerHTML = `
    <strong>Requisitos:</strong>
    <div>${d.requiere.length ? d.requiere.join(", ") : "No tiene"}</div>
    <strong>Es requisito de:</strong>
    <div>${d.esRequisitoDe.length ? d.esRequisitoDe.join(", ") : "No es requisito de otros"}</div>
  `;
...
function crearMateria(nombre,id){
  ...
  const info=infoRamos[nombre] || {creditos:0};
